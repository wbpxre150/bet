...
bits320 curve25519_fmul(const bits320 in2,const bits320 in)
{
...
bits256 curve25519(bits256 mysecret,bits256 basepoint)
{
    ...
    return(fcontract(curve25519_fmul(x,crecip(z))));
}
...
bits320 curve25519_fmul(const bits320 in,const bits320 in2)
{
...
inline bits320 crecip(const bits320 z)
{
    ...
    /* 9 */ b = curve25519_fmul(t0, z); // b = 9
    /* 11 */ a = curve25519_fmul(b, a); // a = 11
    /* 22 */ t0 = fsquare_times(a, 1);
    /* 2^5 - 2^0 = 31 */ b = curve25519_fmul(t0, b);
    /* 2^10 - 2^5 */ t0 = fsquare_times(b, 5);
    /* 2^10 - 2^0 */ b = curve25519_fmul(t0, b);
    /* 2^20 - 2^10 */ t0 = fsquare_times(b, 10);
    /* 2^20 - 2^0 */ c = curve25519_fmul(t0, b);
    /* 2^40 - 2^20 */ t0 = fsquare_times(c, 20);
    /* 2^40 - 2^0 */ t0 = curve25519_fmul(t0, c);
    /* 2^50 - 2^10 */ t0 = fsquare_times(t0, 10);
    /* 2^50 - 2^0 */ b = curve25519_fmul(t0, b);
    /* 2^100 - 2^50 */ t0 = fsquare_times(b, 50);
    /* 2^100 - 2^0 */ c = curve25519_fmul(t0, b);
    /* 2^200 - 2^100 */ t0 = fsquare_times(c, 100);
    /* 2^200 - 2^0 */ t0 = curve25519_fmul(t0, c);
    /* 2^250 - 2^50 */ t0 = fsquare_times(t0, 50);
    /* 2^250 - 2^0 */ t0 = curve25519_fmul(t0, b);
    /* 2^255 - 2^5 */ t0 = fsquare_times(t0, 5);
    /* 2^255 - 21 */ return(curve25519_fmul(t0, a));
}
